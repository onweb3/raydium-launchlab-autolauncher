...
exports.cleanupAllEmptyTokenAccounts = cleanupAllEmptyTokenAccounts;
exports.listAllTokenAccounts = listAllTokenAccounts;
exports.forceCloseTokenAccount = forceCloseTokenAccount;
exports.cleanupOnlyWSOLAccounts = cleanupOnlyWSOLAccounts;
const web3_js_1 = require("@solana/web3.js");
const spl_token_1 = require("@solana/spl-token");
const constant_1 = require("../constant");
// WSOL mint address
const WSOL_MINT = spl_token_1.NATIVE_MINT;
/**
 * Closes all empty token accounts (balance = 0) including WSOL accounts
 * Reclaims rent from closed accounts
 */
async function cleanupAllEmptyTokenAccounts() {
    try {
        console.log('🧹 Starting cleanup of all empty token accounts...');
        console.log(`Wallet: ${constant_1.OWNER.publicKey.toString()}\n`);
        // Get all token accounts owned by the wallet
        const tokenAccounts = await constant_1.CONNECTION.getParsedTokenAccountsByOwner(constant_1.OWNER.publicKey, { programId: spl_token_1.TOKEN_PROGRAM_ID });
        ...
        for (const { pubkey, account } of tokenAccounts.value) {
            ...
            const mint = new web3_js_1.PublicKey(parsedInfo.mint);
            ...
            if (rawBalance === '0' || balance === 0) {
                ...
                try {
                    // Close the account
                    const signature = await (0, spl_token_1.closeAccount)(constant_1.CONNECTION, constant_1.OWNER, // Payer
                    pubkey, // Token account to close
                    constant_1.OWNER.publicKey, // Destination for rent
                    constant_1.OWNER, // Authority
 ...
 */
async function listAllTokenAccounts() {
    try {
 ...
 */
async function forceCloseTokenAccount(accountAddress) {
    try {
        ...
        const signature = await (0, spl_token_1.closeAccount)(constant_1.CONNECTION, constant_1.OWNER, accountAddress, constant_1.OWNER.publicKey, constant_1.OWNER, [], { commitment: 'confirmed' });
 ...
 */
async function cleanupOnlyWSOLAccounts() {
    try {
        console.log('🧹 Cleaning up WSOL accounts specifically...\n');
        const tokenAccounts = await constant_1.CONNECTION.getParsedTokenAccountsByOwner(constant_1.OWNER.publicKey, { mint: WSOL_MINT });
        ...
        for (const { pubkey, account } of tokenAccounts.value) {
            ...
            try {
                const signature = await (0, spl_token_1.closeAccount)(constant_1.CONNECTION, constant_1.OWNER, pubkey, constant_1.OWNER.publicKey, constant_1.OWNER, [], { commitment: 'confirmed' });
                console.log(`   ✅ Closed! TX: ${signature}\n`);
                closedCount++;
